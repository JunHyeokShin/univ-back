...
public class EnrollmentController {
  ...
  public ResponseEntity<Void> cancel(
    ...
    this.enrollmentService.cancel(id, user.userId());
  ...
  public ResponseEntity<List<EnrollmentResponse>> myEnrollments(
      @AuthenticationPrincipal AuthUser user) {
    return ResponseEntity.ok(this.enrollmentService.findMyEnrollments(user.userId()));
  }

  @PreAuthorize("hasRole('PROFESSOR') or hasRole('ADMIN')")
  @GetMapping("/courses/{id}")
  public ResponseEntity<List<EnrollmentResponse>> byCourse(@PathVariable Long id) {
    return ResponseEntity.ok(this.enrollmentService.findByCourse(id));
  }
